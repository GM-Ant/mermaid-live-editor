	...
	test('should open chat bubble, send message, and accept tool call', async ({ page }) => {
		...
		await page.route('/api/chat', async (route) => {
            ...
            const chunks = [
                ...
                { choices: [{ delta: { tool_calls: [{ index: 0, id: toolCallId, type: 'function', function: { name: functionName, arguments: "" } }] } }] },
                { choices: [{ delta: { tool_calls: [{ index: 0, function: { arguments: functionArgs } }] } }] }
            ];
